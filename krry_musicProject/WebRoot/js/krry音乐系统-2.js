...
$(function(){
						...
						for(var i = 0;i < lrcArr.length;i++){
							...
							if(ms >= 0){
								if(!isNaN(bofo)){ //如果是数字
									...
									if(lrclast != "\n"){
										html += "<li class='"+classeName+"'><p class='l_text'>"+lrclast+"</p><p class='l_cover'>"+lrclast+"</p></li><br>";
									}
								}
							}
						}
			...
			audioDom.addEventListener("timeupdate",function(){
				...
				var ms = ($(".l_"+s).find(".l_text").eq(0).text().length-7) || 7;
				$(".l_"+s).siblings().find(".l_cover").removeAttr("style");//移除红色歌词的样式
